...
const updateCard = async (req, res)=>{
    ...
    const card1 = new Card(description, number);
    await session_sql.sql(`UPDATE Catan.cards SET card_description = \'${card1.description}\' , card_number = ${card1.number} WHERE card_id = ${id};`).execute();
    session_sql.close();
    res.redirect('/');
}
//delete card--delete
